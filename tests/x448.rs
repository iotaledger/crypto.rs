...
use crypto::keys::x448::{PublicKey, SecretKey};
...
#[test]
fn test_x448_rfc7748() -> crypto::Result<()> {
    let tvs = include!("fixtures/x448.rs");
    ...
    for tv in tvs.iter() {
        ...
        assert_eq!(secret_a.diffie_hellman(&public_b).as_bytes(), &expected[..],);

        if let Some(ref secret_b) = secret_b {
            assert_eq!(
                secret_b.diffie_hellman(&secret_a.public_key()).as_bytes(),
                &expected[..],
            );
        }
    }
